...
function inicializarIncome(dadosIncome) {
    ...
    const totalGeral = dadosIncome.reduce((sum, item) => sum + (item.valor || 0), 0);
    console.log("💰 Total geral de Income:", totalGeral);
    ...
    if (fonteTotalIncomeEl) {
        ...
        console.log("✅ Total de Income atualizado:", fonteTotalIncomeEl.textContent);
    } else {
        console.error("❌ Erro: elemento fonteTotalIncome não encontrado");
    }
...
function filtrarIncome() {
    ...
    // Atualizar total
    const totalFiltrado = dadosFiltrados.reduce((sum, item) => sum + (item.valor || 0), 0);
    console.log("💰 Total filtrado de Income:", totalFiltrado, "de", dadosFiltrados.length, "itens");
    const fonteTotalIncomeEl = document.getElementById('fonteTotalIncome');
    if (fonteTotalIncomeEl) {
        fonteTotalIncomeEl.textContent = `R$ ${totalFiltrado.toLocaleString('pt-BR', {minimumFractionDigits: 2})}`;
        console.log("✅ Total filtrado de Income atualizado:", fonteTotalIncomeEl.textContent);
    } else {
