...
function criarGraficoPizza(categorias) {
    ...
    const cores = [
        '#FF8A95', '#7EC8E3', '#FFE66D', '#95E1A3', '#FFB84D',
        '#D4A5E8', '#FFA5B0', '#FFB85C', '#FF8C42', '#7FCDBB',
        '#C77DFF', '#6EC5B8', '#FFD93D', '#A78BFA', '#5FA8D3'
    ];
...
function filtrarCategoriasPrincipais(categoriasOriginais) {
    ...
    categoriasFiltradas = categoriasFiltradas.map(cat => {
        ...
        const itensFiltrados = cat.itens.filter(item => {
            // Verificar primeiro se é item anual e se o filtro anual está ativo
            const isAnual = item.frequencia && item.frequencia.toLowerCase().trim() === 'anual';
            const filtroAnualAtivo = filtroCobranca && filtroCobranca.trim() === 'anual';
            
            // Se o filtro anual está ativo e o item é anual, passar por todos os filtros exceto cobrança
            if (filtroAnualAtivo && isAnual) {
                ...
                return passaCarteira && passaFonte && passaBusca;
            }
            
            // Para itens não anuais ou quando o filtro anual não está ativo, aplicar todos os filtros normalmente
            let passaCarteira = !filtroCarteira || item.carteira === filtroCarteira;
            let passaFonte = !filtroFonte || item.fonte === filtroFonte;
            ...
            if (filtroCobranca && filtroCobranca !== 'todas') {
                if (filtroCobranca === 'simples') {
                    passaCobranca = item.frequencia && item.frequencia.toLowerCase().trim() === 'simples';
                } else if (filtroCobranca === 'parceladas') {
                    passaCobranca = item.frequencia && item.frequencia.toLowerCase().trim() === 'parcelada';
                } else if (filtroCobranca === 'fixa') {
                    passaCobranca = item.tipo && item.tipo.toLowerCase().trim() === 'fix';
                } else if (filtroCobranca === 'anual') {
                    passaCobranca = isAnual;
                }
            }
            ...
            let passaMes = true;
            if (filtroMes) {
                if (isAnual && item.meses_parcelas && item.meses_parcelas.length > 0) {
                    passaMes = item.meses_parcelas.some(mes => mes.chave === filtroMes);
                } else if (item.mes_chave) {
            ...
            if (filtroTipoCompra === 'unicas') {
                passaTipoCompra = !item.parcela_atual;
            } else if (filtroTipoCompra === 'parceladas') {
                passaTipoCompra = item.parcela_atual !== null;
            }
...
function inicializarIncome(dadosIncome) {
    ...
    // Preencher select de recorrência (usar frequencia: Mensal, Anual)
    const frequenciasSet = new Set();
    dadosIncome.forEach(item => {
        if (item.frequencia) {
            // Normalizar: mensal -> Mensal, anual -> Anual
            const freq = item.frequencia.toLowerCase();
            if (freq === 'mensal') {
                frequenciasSet.add('Mensal');
            } else if (freq === 'anual') {
                frequenciasSet.add('Anual');
            } else {
                frequenciasSet.add(item.frequencia);
            }
        }
    });
    const frequenciasArray = Array.from(frequenciasSet).sort();
    const filterRecorrenciaIncomeEl = document.getElementById('filterRecorrenciaIncome');
    if (filterRecorrenciaIncomeEl) {
        frequenciasArray.forEach(frequencia => {
            const option = document.createElement('option');
            option.value = frequencia.toLowerCase(); // valor em lowercase para comparação
            option.textContent = frequencia; // texto capitalizado
            filterRecorrenciaIncomeEl.appendChild(option);
        });
    }
...
function resetarFiltrosOutcome(categoriasArray) {
    ...
    if (filterMesEl) {
        ...
        // Verificar se o mês atual existe nas opções do select
        const opcoes = Array.from(filterMesEl.options);
        const mesExiste = opcoes.some(option => option.value === mesChaveAtual);
        
        if (mesExiste) {
            filterMesEl.value = mesChaveAtual;
        } else {
            // Se não existir, limpar o filtro
            filterMesEl.value = '';
        }
