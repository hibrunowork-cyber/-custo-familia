...
function inicializarDashboard(dados) {
    ...
    // Filtro de despesa fixa
    document.getElementById('filterDespesaFixa').addEventListener('change', () => {
        filtrarTudo(categoriasArray);
    });
    ...
    document.getElementById('btnResetarFiltros').addEventListener('click', () => {
        ...
        document.getElementById('filterTipoCompra').value = 'todas';
        document.getElementById('filterDespesaFixa').value = 'todas';
        filtrarTudo(categoriasArray);
    });
...
function filtrarCategoriasPrincipais(categoriasOriginais) {
    ...
    const filtroDespesaFixa = document.getElementById('filterDespesaFixa').value;
    ...
    if (filtroFonte || filtroMes || filtroTipoCompra !== 'todas' || filtroDespesaFixa !== 'todas' || filtroBusca) {
        categoriasFiltradas = categoriasFiltradas.map(cat => {
            const itensFiltrados = cat.itens.filter(item => {
                ...
                // Filtro por despesa fixa
                if (filtroDespesaFixa === 'sim') {
                    passaDespesaFixa = item.despesa_fixa === 'fix';
                } else if (filtroDespesaFixa === 'nao') {
                    passaDespesaFixa = item.despesa_fixa === 'nfix';
                }
                ...
                return passaFonte && passaMes && passaTipoCompra && passaDespesaFixa && passaBusca;
            });
