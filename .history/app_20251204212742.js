...
function inicializarIncome(dadosIncome) {
    ...
    // Preencher select de recorrência
    const recorrenciasSet = new Set();
    dadosIncome.forEach(item => {
        if (item.recorrencia) recorrenciasSet.add(item.recorrencia);
    });
    const recorrenciasArray = Array.from(recorrenciasSet).sort();
    ...
    if (filterRecorrenciaIncomeEl) {
        recorrenciasArray.forEach(recorrencia => {
            const option = document.createElement('option');
            option.value = recorrencia;
            option.textContent = recorrencia;
