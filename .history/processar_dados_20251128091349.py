import json
import re
import os

def processar_linha(linha):
    """Processa uma linha do arquivo DADOS_07.txt"""
    partes = linha.strip().split('|')
    
    if len(partes) != 4:
        return None
    
    fonte, gasto, valor_str, parcelas_str = partes
    
    # Pular linha de cabeçalho
    if fonte == "Fonte":
        return None
    
    # Processar valor
    try:
        # Remove pontos de milhar e troca vírgula por ponto
        valor_limpo = valor_str.strip()
        # Se tem vírgula, é o separador decimal brasileiro
        if ',' in valor_limpo:
            valor_limpo = valor_limpo.replace('.', '').replace(',', '.')
        valor = float(valor_limpo)
    except:
        valor = 0.0
    
    # Processar categoria e subcategoria
    if ' / ' in gasto:
        partes_gasto = gasto.split(' / ', 1)
        categoria = partes_gasto[0].strip()
        subcategoria = partes_gasto[1].strip()
        
        # Normalizar algumas categorias
        if categoria == "ASSIN":
            categoria = "ASSINATURAS"
        elif categoria == "LAZER":
            categoria = "COMPRAS"  # Conforme vi nos dados processados
        
    else:
        categoria = gasto.strip()
        subcategoria = gasto.strip()
    
    categoria_completa = f"{categoria} / {subcategoria}" if categoria != subcategoria else categoria
    
    # Processar parcelas
    parcela_atual = None
    parcelas_total = None
    
    if parcelas_str and parcelas_str != '-':
        # Tentar extrair números de parcelas
        match = re.search(r'(\d+)[/\s]+(\d+)', parcelas_str)
        if match:
            parcela_atual = int(match.group(1))
            parcelas_total = int(match.group(2))
    
    # Montar objeto
    return {
        "linha_original": linha.strip().replace('|', ' '),
        "fonte": fonte,
        "categoria_completa": categoria_completa,
        "categoria": categoria,
        "subcategoria": subcategoria,
        "valor": valor,
        "parcela_atual": parcela_atual,
        "parcelas_total": parcelas_total
    }

def processar_dados_07():
    """Processa o arquivo DADOS_07.txt e gera dados_processados.json"""
    
    print("🔄 Processando arquivo DADOS_07.txt...")
    
    # Verificar quantidade anterior de registros
    controle_file = '.ultimo_processamento.json'
    ultima_quantidade = 0
    
    if os.path.exists(controle_file):
        try:
            with open(controle_file, 'r', encoding='utf-8') as f:
                controle = json.load(f)
                ultima_quantidade = controle.get('total_registros', 0)
        except:
            ultima_quantidade = 0
    
    dados = []
    
    # Ler arquivo DADOS_07.txt
    with open('src/DADOS_07.txt', 'r', encoding='utf-8') as f:
        for linha in f:
            linha = linha.strip()
            if not linha:
                continue
            
            item = processar_linha(linha)
            if item:
                dados.append(item)
    
    # Marcar os novos registros
    total_atual = len(dados)
    novos_registros = total_atual - ultima_quantidade
    
    if novos_registros > 0:
        print(f"🆕 Detectados {novos_registros} novos lançamentos!")
        # Marcar os últimos N registros como novos
        for i in range(max(0, total_atual - novos_registros), total_atual):
            dados[i]['novo'] = True
    else:
        # Nenhum registro é novo
        for item in dados:
            item['novo'] = False
    
    # Salvar dados processados
    with open('dados_processados.json', 'w', encoding='utf-8') as f:
        json.dump(dados, f, ensure_ascii=False, indent=2)
    
    # Salvar controle
    with open(controle_file, 'w', encoding='utf-8') as f:
        json.dump({'total_registros': total_atual}, f)
    
    print(f"✅ Processamento concluído!")
    print(f"📊 Total de registros: {len(dados)}")
    print(f"💰 Total: R$ {sum(d['valor'] for d in dados):,.2f}")
    
    return dados

if __name__ == '__main__':
    processar_dados_07()

