...
def processar_linha(linha):
    ...
    # Calcular mês do lançamento
    # Assumindo que estamos em Novembro/2024 (mês 11)
    mes_atual = datetime.now().month
    ano_atual = datetime.now().year
    
    if parcela_atual and parcelas_total:
        # Para parceladas, a parcela atual é do mês atual
        # Então este lançamento é do mês atual
        mes_lancamento = mes_atual
        ano_lancamento = ano_atual
    else:
        # Para compras únicas, é do mês atual
        mes_lancamento = mes_atual
        ano_lancamento = ano_atual
    
    mes_chave = f"{ano_lancamento}-{str(mes_lancamento).zfill(2)}"
    nome_mes = datetime(ano_lancamento, mes_lancamento, 1).strftime('%B').capitalize()
