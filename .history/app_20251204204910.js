...
function filtrarCategoriasPrincipais(categoriasOriginais) {
    ...
    categoriasFiltradas = categoriasFiltradas.map(cat => {
        ...
        const itensFiltrados = cat.itens.filter(item => {
            ...
            // Filtro de cobrança (verificar primeiro para itens anuais)
            ...
            if (filtroCobranca && filtroCobranca !== 'todas') {
                if (filtroCobranca === 'simples') {
                    ...
                    // Debug temporário
                    if (isAnual) {
                        console.log('Item anual encontrado:', item);
                    }
                }
            }
            
            // Filtro de mês: verificar se o item tem o mês em meses_parcelas ou se tem mes_chave
            // Se o filtro de cobrança for "anual", ignorar o filtro de mês para mostrar todos os itens anuais
            let passaMes = true;
            if (filtroMes && !filtroAnualAtivo) {
                // Para itens anuais quando não há filtro anual ativo, verificar meses_parcelas
                if (isAnual && item.meses_parcelas && item.meses_parcelas.length > 0) {
                    // Verificar se algum mês corresponde ao filtro
            ...
            // Se o filtro de cobrança for "anual", ignorar o filtro de tipo de compra para itens anuais
            if (!filtroAnualAtivo || !isAnual) {
                if (filtroTipoCompra === 'unicas') {
                    // Compras sem parcelas: itens sem parcela_atual (inclui anuais)
                    passaTipoCompra = !item.parcela_atual;
                } else if (filtroTipoCompra === 'parceladas') {
                    // Compras parceladas: itens com parcela_atual
